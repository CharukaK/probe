...
unsigned tree_sitter_probe_external_scanner_serialize(void *p, char *b) {
  (void)p;
  (void)b;
  return 0;
}
void tree_sitter_probe_external_scanner_deserialize(void *p, const char *b,
                                                    unsigned n) {
  (void)p;
  (void)b;
  (void)n;
...
bool tree_sitter_probe_external_scanner_scan(void *payload, TSLexer *lexer,
                                             const bool *valid_symbols) {
  (void)payload;
  if (!valid_symbols[OCTET_BODY])
    return false;
  ...
  for (;;) {
    ...
    if (lexer->eof(lexer))
      break;
    if (at_line_start) {
      if (consume_boundary_if_present(lexer))
        break; // real boundary — stop, exclude it
      at_line_start = false;
  ...
  if (!consumed_any)
    return false; // body started right at a boundary — decline entirely
